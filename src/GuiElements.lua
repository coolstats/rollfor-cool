...
function M.icon( parent, show, width, height )
  ...
  icon:SetTexture( m.asset_path( "icon-white2.tga" ) )
...
function M.tiny_button( parent, text, tooltip, color, font_size )
  ...
  if m.classic then
    ...
    if text == 'X' then
      ...
      button:SetNormalTexture( m.asset_path( "tiny-button-up.tga" ) )
      button:SetPushedTexture( m.asset_path( "tiny-button-down.tga" ) )
    end
...
function M.resize_grip( parent, on_start, on_end )
  ...
  button:SetWidth( 16 )
  button:SetHeight( 16 )
  button:SetNormalTexture( m.asset_path( "resize-grip.tga" ), "ARTWORK" )
...
function M.titlebar( parent, title, on_close )
  ...
  if not m.classic then
    ...
    frame:SetBackdrop( {
      bgFile = m.asset_path( "titlebar-top.tga" ),
      tile = true,
      tileSize = 32,
      edgeSize = 0,
    ...
    topLeft:SetTexture( m.asset_path( "titlebar-topleft.tga" ) )
    topLeft:SetPoint( "TOPLEFT", frame, "TOPLEFT", 0, 0 )
    topLeft:SetWidth( 64 )
    topLeft:SetHeight( 32 )

    local topRight = frame:CreateTexture( nil, "BORDER" )
    topRight:SetTexture( m.asset_path( "titlebar-topright.tga" ) )
...
function M.info( parent )
  ...
  icon:SetWidth( 11 )
  icon:SetHeight( 11 )
  icon:SetTexture( m.asset_path( "info.tga" ) )

...
function M.asset_path( file_name )
  return string.format( "Interface\\AddOns\\%s\\assets\\%s", M.addon_name, file_name )
end
...
function M.roll_type_color( roll_type, text )
  ...
  elseif roll_type == M.Types.RollType.SoftRes then
    ...
    return M.colors.white( text or tostring( roll_type or "unknown" ) )
  end
end

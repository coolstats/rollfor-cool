...
function M.new( player_info, softres )
  ...
  local function reply_with_sr( sender )
  ...
  -- ".loot <command>" whisper commands. Add new entries here to support more commands.
  local loot_commands = {
    sr = reply_with_sr,
  }
  ...
  local function on_chat_msg_whisper( message, sender )
    ...
    if string.find( lower_message, "^%?sr" ) then
      reply_with_sr( sender )
      return
    end

    local command = lower_message:match( "^%.loot%s+(%S+)%s*$" )
    if command and loot_commands[ command ] then
      loot_commands[ command ]( sender )
    end
  end

...
-- Re-imports the currently loaded (encoded) soft-res data, so that changes to the item
-- replacements mapping/toggle are reflected immediately without requiring a fresh import.
local function refresh_softres_data()
  if M.softres_db and M.softres_db.data then
...
local function create_components()
  ...
  if M.softres_db.use_item_names == nil then M.softres_db.use_item_names = true end
  M.use_item_names = {
    is_enabled = function() return M.softres_db.use_item_names end,
    set_enabled = function( value ) M.softres_db.use_item_names = value and true or false end,
  }

  M.unfiltered_softres = m.SoftRes.new( M.softres_db, M.use_item_names, M.ace_timer )
  ...
  -- Create LootAwardCallback first with nil for the controller.
  ...
  -- Create RollController and pass in the callback as the ninth argument.
  ...
  -- Link the controller back to the callback using the setter.
  ...
  M.softres_gui = m.SoftResGui.new( M.api, M.import_encoded_softres_data, M.softres_check, M.softres, clear_data, M.dropped_loot_announce.reset, M.ace_timer, M.group_roster, M.unfiltered_softres, M.use_item_names, refresh_softres_data )
...
local function is_rolling_check( f )
  return function( ... )
    ...
    -- f( unpack( args ) )
    f( ...) -- this line was patched/changed by Cronoh.  Do not attempt to unpack arguments - the functions are not expecting them, and they do not exist
  end
end
...
local function on_check_softres_command( args )
  if string.find( args, "^a" ) then
    ...
    if result == M.softres_check.ResultType.SomeoneIsNotSoftRessing and m.raid_id then
      local msg = m.raid_url or string.format( "Soft-res id: %s", m.raid_id )
...
local function on_check_softres_announce_command()
  on_check_softres_command( "aw" )
end
...
local function setup_slash_commands()
  ...
  SLASH_SRCA1 = "/srca"
  M.api().SlashCmdList[ "SRCA" ] = on_check_softres_announce_command
  SLASH_SRO1 = "/sro"
